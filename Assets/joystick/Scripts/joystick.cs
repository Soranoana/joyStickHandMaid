...
public class joystick : Graphic, IPointerDownHandler, IPointerUpHandler, IEndDragHandler, IDragHandler {
    ...
    private Vector3 _stickPosition {
        set {
            ...
            _position = new Vector2(_stick.transform.localPosition.x / _radius,
...
#elif UNITY_IOS
        GetTouchesIdFromNearFinger();
        RectTransformUtility.ScreenPointToLocalPointInRectangle(GetComponent<RectTransform>(),
          new Vector2(Input.touches[touchesID].position.x, Input.touches[touchesID].position.y),
          null,
          out screenPos
        );
    ...
    touchesID判別（近いものを判別）
    =================================================================================*/
    private void GetTouchesIdFromNearFinger() {
        touchesID = 0;
        for (int i = 1; i < Input.touchCount; i++) {
            if (Vector2.Distance(transform.position, Input.touches[touchesID].position) > Vector2.Distance(transform.position, Input.touches[i].position)) {
                touchesID = i;
            }
        }
    }
